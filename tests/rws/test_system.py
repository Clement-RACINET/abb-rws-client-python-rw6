# This file is AUTO-GENERATED by contrib/generator/main.py
# DO NOT EDIT MANUALLY — run the generator to regenerate.
"""Tests unitaires auto-générés pour rws/system."""
from __future__ import annotations

import pytest
import httpx

from abb_rws_client._core.client import RWSClient
from abb_rws_client.rws.system import (
    get_system_information,
    get_robot_type,
    get_system_options,
    get_system_energy_actions,
    get_system_energy_info_change_count,
    reset_accumulated_energy,
    get_system_energy,
    subscribe_on_system_energy_changes,
    get_system_robotware_license,
    get_installed_products,
)


class _MockTransport(httpx.AsyncBaseTransport):
    """Transport mock retournant une réponse HTTP configurable."""

    def __init__(self, status_code: int = 200, content: bytes = b"") -> None:
        self.status_code = status_code
        self.content = content
        self.last_request: httpx.Request | None = None

    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        self.last_request = request
        return httpx.Response(self.status_code, content=self.content, request=request)


def _make_client(transport: _MockTransport) -> RWSClient:
    """Construit un RWSClient branché sur le transport mock."""
    client = RWSClient.__new__(RWSClient)
    client._http = httpx.AsyncClient(
        transport=transport,
        base_url="http://localhost/",
    )
    client.host = "localhost"
    return client


@pytest.mark.asyncio
async def test_get_system_information() -> None:
    """Vérifie que get_system_information émet GET /rw/system."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_system_information(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_robot_type() -> None:
    """Vérifie que get_robot_type émet GET /rw/system/robottype."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_robot_type(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/robottype"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_system_options() -> None:
    """Vérifie que get_system_options émet GET /rw/system/options."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_system_options(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/options"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_system_energy_actions() -> None:
    """Vérifie que get_system_energy_actions émet GET /rw/system/energy."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_system_energy_actions(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/energy"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_system_energy_info_change_count() -> None:
    """Vérifie que get_system_energy_info_change_count émet GET /rw/system/energy."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_system_energy_info_change_count(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/energy"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_reset_accumulated_energy() -> None:
    """Vérifie que reset_accumulated_energy émet POST /rw/system/energy."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await reset_accumulated_energy(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/system/energy"
    assert resp.status_code == 204

@pytest.mark.asyncio
async def test_get_system_energy() -> None:
    """Vérifie que get_system_energy émet GET /rw/system/energy."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_system_energy(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/energy"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_subscribe_on_system_energy_changes() -> None:
    """Vérifie que subscribe_on_system_energy_changes émet POST /subscription."""
    transport = _MockTransport(status_code=201)
    client = _make_client(transport)

    resp = await subscribe_on_system_energy_changes(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/subscription"
    assert resp.status_code == 201

@pytest.mark.asyncio
async def test_get_system_robotware_license() -> None:
    """Vérifie que get_system_robotware_license émet GET /rw/system/license."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_system_robotware_license(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/license"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_installed_products() -> None:
    """Vérifie que get_installed_products émet GET /rw/system/products."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_installed_products(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/system/products"
    assert resp.status_code == 200
