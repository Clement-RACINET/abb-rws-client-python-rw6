# This file is AUTO-GENERATED by contrib/generator/main.py
# DO NOT EDIT MANUALLY — run the generator to regenerate.
"""Tests unitaires auto-générés pour rws/rapid/execution."""
from __future__ import annotations

import pytest
import httpx

from abb_rws_client._core.client import RWSClient
from abb_rws_client.rws.rapid.execution import (
    get_rapid_execution_state,
    get_rapid_execution_actions,
    start_rapid_execution,
    stop_rapid_execution,
    start_rapid_execution_from_production_entry,
    reset_rapid_program_pointer_to_main,
    set_number_of_execution_cycles,
    subscribe_rapid_execution,
    subscribe_rapid_execution_cycle,
    subscribe_on_hold_to_run,
    set_hold_to_run_cmd,
)


class _MockTransport(httpx.AsyncBaseTransport):
    """Transport mock retournant une réponse HTTP configurable."""

    def __init__(self, status_code: int = 200, content: bytes = b"") -> None:
        self.status_code = status_code
        self.content = content
        self.last_request: httpx.Request | None = None

    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        self.last_request = request
        return httpx.Response(self.status_code, content=self.content, request=request)


def _make_client(transport: _MockTransport) -> RWSClient:
    """Construit un RWSClient branché sur le transport mock."""
    client = RWSClient.__new__(RWSClient)
    client._http = httpx.AsyncClient(
        transport=transport,
        base_url="http://localhost/",
    )
    client.host = "localhost"
    return client


@pytest.mark.asyncio
async def test_get_rapid_execution_state() -> None:
    """Vérifie que get_rapid_execution_state émet GET /rw/rapid/execution."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_rapid_execution_state(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_get_rapid_execution_actions() -> None:
    """Vérifie que get_rapid_execution_actions émet GET /rw/rapid/execution."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_rapid_execution_actions(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 200

@pytest.mark.asyncio
async def test_start_rapid_execution() -> None:
    """Vérifie que start_rapid_execution émet POST /rw/rapid/execution."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await start_rapid_execution(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 204

@pytest.mark.asyncio
async def test_stop_rapid_execution() -> None:
    """Vérifie que stop_rapid_execution émet POST /rw/rapid/execution."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await stop_rapid_execution(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 204

@pytest.mark.asyncio
async def test_start_rapid_execution_from_production_entry() -> None:
    """Vérifie que start_rapid_execution_from_production_entry émet POST /rw/rapid/execution."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await start_rapid_execution_from_production_entry(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 204

@pytest.mark.asyncio
async def test_reset_rapid_program_pointer_to_main() -> None:
    """Vérifie que reset_rapid_program_pointer_to_main émet POST /rw/rapid/execution."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await reset_rapid_program_pointer_to_main(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 204

@pytest.mark.asyncio
async def test_set_number_of_execution_cycles() -> None:
    """Vérifie que set_number_of_execution_cycles émet POST /rw/rapid/execution."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await set_number_of_execution_cycles(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 204

@pytest.mark.asyncio
async def test_subscribe_rapid_execution() -> None:
    """Vérifie que subscribe_rapid_execution émet POST /subscription."""
    transport = _MockTransport(status_code=201)
    client = _make_client(transport)

    resp = await subscribe_rapid_execution(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/subscription"
    assert resp.status_code == 201

@pytest.mark.asyncio
async def test_subscribe_rapid_execution_cycle() -> None:
    """Vérifie que subscribe_rapid_execution_cycle émet POST /subscription."""
    transport = _MockTransport(status_code=201)
    client = _make_client(transport)

    resp = await subscribe_rapid_execution_cycle(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/subscription"
    assert resp.status_code == 201

@pytest.mark.asyncio
async def test_subscribe_on_hold_to_run() -> None:
    """Vérifie que subscribe_on_hold_to_run émet POST /subscription."""
    transport = _MockTransport(status_code=201)
    client = _make_client(transport)

    resp = await subscribe_on_hold_to_run(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/subscription"
    assert resp.status_code == 201

@pytest.mark.asyncio
async def test_set_hold_to_run_cmd() -> None:
    """Vérifie que set_hold_to_run_cmd émet POST /rw/rapid/execution."""
    transport = _MockTransport(status_code=204)
    client = _make_client(transport)

    resp = await set_hold_to_run_cmd(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "POST"
    assert transport.last_request.url.path == "/rw/rapid/execution"
    assert resp.status_code == 204
