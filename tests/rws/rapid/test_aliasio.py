...
"""Tests unitaires auto-générés pour rws/rapid/aliasio."""
...
from abb_rws_client.rws.rapid.aliasio import (
    get_aliasio_list,
)
...
async def test_get_aliasio_list() -> None:
    """Vérifie que get_aliasio_list émet GET /rw/rapid/aliasio."""
    transport = _MockTransport(status_code=200)
    client = _make_client(transport)

    resp = await get_aliasio_list(client)

    assert transport.last_request is not None
    assert transport.last_request.method == "GET"
    assert transport.last_request.url.path == "/rw/rapid/aliasio"
